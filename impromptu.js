...
function startImpromptuPrep() {
    document.getElementById("main").innerHTML = `
    <DIV CLASS="timer-small timer" ID="timer1">1:00</DIV>
    <SPAN CLASS="timer-alt">The timer is hidden.</SPAN> Your prompts (you have one minute to choose one and prep):
    ${generateTopic().replace(/^"(.*)"$/, '$1').replace(/""/g, "\"").replace(/""/g, "\"")}
    `
    toggleTimer();

    startTimer(60, document.getElementById("timer1"), statePrompt)
}
...
function statePrompt() {
    document.getElementById("main").innerHTML = `
    Stop! State your prompt, but don't begin until this screen changes in <B><SPAN ID="timer2" CLASS="timer">0:10</SPAN></B> seconds.`
    startTimer(10, document.getElementById("timer2"), startImpromptuSpeech)
}

function startImpromptuSpeech() {
    document.getElementById("main").style.textAlign = "center";
    document.getElementById("main").innerHTML = `
    <SPAN ID="timer3" CLASS="timer-large timer">2:00</SPAN>
    <SPAN ID="timeleft" CLASS="timer-alt">2 minutes left</SPAN>
    <BR>
    <BUTTON CLASS="main-button" ONCLICK="finishImpromptuSpeech()">Finish speech</BUTTON>
    `
    toggleTimer();
    startTimer(120, document.getElementById("timer3"));
...
function finishImpromptuSpeech() {
    let time = document.getElementById("timer3").innerText;
    time = time.split(":");
    let minutesSpent = 1-Number(time[0]);
    let secondsSpent = 60-Number(time[1]);
    if(secondsSpent == 60) {
        minutesSpent++;
        secondsSpent = 0;
    }
    let totalTime = 120 - (Number(time[0])*60+Number(time[1]));

    clearTimeout(speechTimeout);
    document.body.style.backgroundColor = "var(--dark)";
    document.getElementById("main").style.textAlign = "left";
    document.getElementById("main").innerHTML = `
    Your impromptu speech lasted ${minutesSpent} minutes and ${secondsSpent} seconds. You have ${document.getElementById("timer3").innerText} left over. <SPAN ID="append-note"></SPAN>
    <BR><BUTTON CLASS="main-button" ONCLICK="homePage()" STYLE="margin-top: 15px">Back to home page</BUTTON>
    `
    if(totalTime == 120) {
        document.getElementById("append-note").innerText = "Watch out! Don't go over time (1.5 to 2 minutes)!";
    } else if(totalTime < 90) {
        document.getElementById("append-note").innerText = "Warning: you did not make time (1.5 to 2 minutes).";
    } else {
        document.getElementById("append-note").innerText = "Congrats, you made time (1.5 to 2 minutes).";
    }
}
...
function homePage() {
    ...
    <DIV CLASS="main-text">Click the button below to start your impromptu practice. You will have 1 minute to select a prompt and prepare, and 1.5 to 2 minutes to give your speech. Signals will be given at the 1, 1.5, and 2 minute mark. This webapp will auto-generate 3 prompts for you; pick one to respond to. <DIV STYLE="height: 10px;"></DIV>If you check "hide timer", you'll only see the color-coded timecards without a timer. Color codes are:
